...
|---|---|---|---|
| Identity | Realign to the canonical palette and the three-node mark, as `registry` did — *copied rather than imported: the repositories share no code, only the identity* | parallel · before F10 | done — the colour, the mark, the ladder, the type and the shape are the platform's. `src/styles/site.css` carries the five-rung ladder (`#080a0e → #0e1116 → #1c212a → #262b36 → #313743`), the identity orange `#eb7229` with its shades mixed from it rather than written out, the state colours the application uses for *well* and *failed*, and the one radius with its two steps. **The webfont is gone**: the type is the system stack, so the site fetches nothing from anywhere and there is one fewer thing between arriving and reading. And it is held there by `tests/identity.test.mjs`, which reads the built stylesheet rather than the source — the failure this guards against is silent, because a colour edited here looks fine on this site and looks like a different product beside the app |
| Reflow | One breakpoint, because a grid that finds its own column count from the width it is given needs none of its own | parallel · before F10 | done — nine widths became one. Every grid on the site is `repeat(auto-fit, minmax(min(100%, max(<floor>, <share>)), 1fr))`, which caps the columns at what the layout wants and drops to one when there is no room, and the chevron between the publishing steps asks a **container** query — how wide that list is — rather than guessing from the window. What is left is `600px`, and the one exception: the navigation, whose fit depends on how long the words are in the language being read. The test above holds the count to exactly that |
