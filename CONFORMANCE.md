...
|---|---|---|---|
| §13.10 | The platform's portal: **what Almena is, for whom, and what it lets you do**. Holds no state, is consulted by no component, and its being down degrades nothing | parallel · before F10 | partial — `src/components/SitePage.astro`, with the words in `src/data/content.ts`, `src/data/diagrams.ts` and `src/data/roadmap.ts`, and the chrome in `src/data/site.ts`; eight numbered sections, eight drawings under `src/components/diagrams/` and the use-case carousel in `src/components/UseCases.astro` |
